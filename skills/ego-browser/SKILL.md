...
For setup, install, or connection problems, direct the user to rerun Freeklaw's tagged-release `./install.sh` (or `./install.sh --check`), which installs and verifies the pinned ego lite build. Do not improvise an ego installation.
...
- If the first run reports `command not found` / a missing environment (most likely ego lite isn't installed yet or `~/.local/bin` is off PATH), stop and direct the user to rerun Freeklaw's tagged-release `./install.sh`, then return to the original task — do not give up, do not improvise a download, and do not keep retrying the same heredoc.
